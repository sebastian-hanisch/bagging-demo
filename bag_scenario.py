...
DOMINANT_FEATURE = {"class": 1, "reg": 0}          # Wurzel in jedem Baum: Ladegewicht (Klassifikation), Distanz (Regression)


def weaken_dominant(X, task, strength, seed):
    """Kopie von X, in der die dominante Spalte (Wurzel jedes Baums) in (100 - strength) % der Zeilen durch eine zufällige Permutation ihrer selbst ersetzt ist: dieselbe Verteilung, aber ohne Bezug zum Ziel in
    diesen Zeilen. strength = 100 lässt X unverändert; strength = 0 zerstört den Bezug ganz. Nur zum Experimentieren (Regler "Stärke des dominanten Merkmals") - kein Teil der eigentlichen Szenario-Erzeugung."""
    if strength >= 100:
        return X
    dom = DOMINANT_FEATURE[task]
    rng = np.random.default_rng([int(seed), 777])
    n = len(X)
    perm = rng.permutation(n)
    replace = rng.random(n) >= float(strength) / 100.0
    Xc = X.copy()
    Xc[replace, dom] = X[perm[replace], dom]
    return Xc
