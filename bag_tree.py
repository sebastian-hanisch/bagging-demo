"""Der CART-Baumkern (wortgleich aus cart-demo übernommen, dort ausführlich gegen Brute-Force und scikit-learn geprüft) - Bagging (bag_algorithm.py) wächst viele dieser Bäume auf Bootstrap-Stichproben.
...
def grow(X, y, task="class", criterion=None, max_depth=None, min_leaf=1):
    ...
    while queue:
        ...
        if depth[t] >= max_depth or imp[t] <= EPS:
            continue
        s = best_split(X[idx], y[idx], criterion, min_leaf)
        if s is None:
            continue
        f, thr, _ = s
        go_left = X[idx, f] <= thr
