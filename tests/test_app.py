"""Rauchtests der Streamlit-Oberfläche per AppTest: Standard, jedes Preset, Aufgabenwechsel, ausgeblendete Regler, Abspielen, Permalink, Experimente auf Abruf, Schlüssel."""
...
from bag_presets import KEPT, PRESET_KEYS
...
def test_default_renders_without_exception_and_gives_one_verdict():
    at = _run()
    assert any("Bagging in Aktion" in m.value for m in at.markdown) and not at.error
    assert any("Deutlich besser als der Einzelbaum" in m.value for m in at.markdown)
...
def test_a_single_tree_shows_no_play_button_and_says_so():
    at = _run(lambda a: _apply(a, C.PRESETS["🌲 Ein Baum"]))
...
def test_extreme_settings_render():
    ...
    def big(at):
        ...
        at.session_state["n_trees_slider"] = 60
    ...
    def reg(at):
        at.session_state["task_select"] = "reg"
        at.session_state["leaf_slider"] = 1
        at.session_state["dominant_slider"] = 0
...
def test_step_slider_returns_to_the_full_forest_when_settings_change():
    at = _run()
    at.slider(key="bag_step").set_value(5)
    at.run()
    assert at.slider(key="bag_step").value == 5
    at.session_state["n_trees_slider"] = 50
    at.run()
    assert not at.exception and at.slider(key="bag_step").value == 50


def test_every_step_of_a_small_forest_renders():
    at = _run(lambda a: _apply(a, C.PRESETS["🌳 Kleiner Wald"]))
    for k in range(1, int(at.slider(key="bag_step").max) + 1):
        at.slider(key="bag_step").set_value(k)
        at.run()
...
def test_permalink_parameters_are_clamped_and_unknown_choices_fall_back():
    ...
    assert at.selectbox(key="task_select").value == C.DEFAULT_TASK and at.slider(key="dominant_slider").value == C.DOMINANT_MIN
...
def test_the_address_bar_mirrors_the_settings():
    at = _run(lambda a: _apply(a, C.PRESETS["🎯 Dominantes Merkmal"]))
    assert str(at.query_params["dom"]) in ("20", "['20']") and str(at.query_params["task"]) in ("reg", "['reg']")


def test_experiments_run_on_demand():
    at = _run()
    assert not any("Sechs Datensätze" in c.value for c in at.caption)
    for key in ("bv_start", "dom_start"):
        at.button(key=key).click()
        at.run()
        assert not at.exception, (key, [e.value for e in at.exception])
    text = " ".join(c.value for c in at.caption)
    for needle in ("Sechs Datensätze mit denselben Regeln", "Bei voller Stärke (100 %) beginnen alle Bäume", "genau das behebt Random Forest"):
...
def test_every_plotly_chart_has_an_explicit_key_and_axes_are_locked():
    ...
    assert sorted(keys) == sorted(["tree_chart", "map_chart", "sat_chart", "importance_chart", "bv_chart", "dom_chart"]), keys
    looped = [c for c in calls if 'key=f"' in c]
    assert len(looped) == 3 and all('_{current}"' in c for c in looped)                              # die Bilder der Abspiel-Schleife tragen den Schritt
    viz = (ROOT / "bag_visualization.py").read_text(encoding="utf-8")
