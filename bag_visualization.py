"""Plotly-Darstellungen: einzelner Baum (klein, zur Einordnung), Karte des Wald-Mittels über zwei Merkmale, Sättigungs-, Bias-Varianz- und Dominanz-Kurven. Alle Achsen sind gesperrt (Touch-Scrollen)."""
...
def build_tree(tree, task, y_range, height=280):
    """Kleines Baumdiagramm ohne Beschriftung (Übersicht, nicht zum Ablesen) - zeigt, wie unterschiedlich einzelne Bootstrap-Bäume aufgebaut sind."""
...
def build_map(bagging, ds, fx, fy, upto=None, sample=None, height=430):
    task = bagging.task
    Xtr = ds.X[ds.train]
    ...
    z = np.round(bag.predict_value(bagging, grid, upto), 3).reshape(XX.shape)
...
def build_saturation(rows, task, single_error, current_k, height=360):
    k = [r["k"] for r in rows]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=k, y=[r["test"] for r in rows], mode="lines+markers", name="Test", line=dict(color=C.COLORS["test"])))
    fig.add_trace(go.Scatter(x=k, y=[r["oob"] for r in rows], mode="lines+markers", name="Out-of-Bag", line=dict(color=C.COLORS["oob"])))
    fig.add_hline(y=single_error, line=dict(color="#888888", dash="dash"), annotation_text="ein einzelner Baum", annotation_position="top right")
    fig.add_vline(x=current_k, line=dict(color="#111111", dash="dot"))
    fig.update_xaxes(title="Zahl der Bäume", type="log" if k[-1] > 30 else "linear")
...
def build_bias_variance(bv, height=340):
    labels = ["Einzelbaum", "Wald"]
    bias2 = [bv["single"]["bias2"], bv["bagging"]["bias2"]]
    var = [bv["single"]["variance"], bv["bagging"]["variance"]]
    fig = go.Figure()
...
def build_dominance(rows, height=340):
    s = [r["strength"] for r in rows]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=s, y=[r["correlation"] for r in rows], mode="lines+markers", name="Korrelation der Bäume", line=dict(color="#1f77b4"), yaxis="y"))
    fig.add_trace(go.Scatter(x=s, y=[r["root_share"] for r in rows], mode="lines+markers", name="Anteil gleiches Wurzelmerkmal", line=dict(color="#2ca02c"), yaxis="y"))
    fig.update_xaxes(title="Stärke des dominanten Merkmals [%]", autorange="reversed")
    fig.update_yaxes(title="Anteil / Korrelation", range=[0, 1.03])
    return lock_axes(fig, height, legend=dict(orientation="h", y=1.12))
